...
(function () {
  ...
  camera.position.set(7.8, 6.2, 10.8);
  ...
  function showDemo() {
    ...
    [0, 2, 6].forEach(function (idx, i) {
      var s = MODELS.STANDARDS[idx];
      var b = MODELS.makeBolt(s, 'hex');
      b.position.set((i - 1) * 2.8, 0.16 + s.len, 2.75);
      world.add(b);
    });
    castAll(world);
  }
  ...
  // ---------- 3D cross-section: cut the wood box in half (clip plane) ----------
  renderer.localClippingEnabled = true;
  var sectionOn = false;
  var clipPlane = new THREE.Plane(new THREE.Vector3(0, 0, -1), -0.001);
  var sectionEntries = [];

  function clipTraverse(obj) {
    obj.traverse(function (m) {
      if (m.isMesh && m.material && !m.userData.__clipped) {
        var mats = Array.isArray(m.material) ? m.material : [m.material];
        var clones = mats.map(function (mm) {
          var c = mm.clone();
          c.clippingPlanes = [clipPlane];
          c.side = THREE.DoubleSide;
          return c;
        });
        m.userData.__clipped = true;
        sectionEntries.push({ mesh: m, orig: m.material, clones: clones });
        m.material = Array.isArray(m.material) ? clones : clones[0];
      }
    });
  }

  function clearSectionClip() {
    sectionEntries.forEach(function (e) {
      e.mesh.material = e.orig;
      e.mesh.userData.__clipped = false;
      e.clones.forEach(function (c) { c.dispose(); });
    });
    sectionEntries = [];
  }
  ...
  function applySectionClip() {
    ...
    world.traverse(function (o) {
      if (o.userData && o.userData.sectionClip) clipTraverse(o);
    });
  }

  el['btn-section'].onclick = function () {
    SFX.init(); SFX.click();
    sectionOn = !sectionOn;
    el['btn-section'].classList.toggle('sel', sectionOn);
    if (sectionOn) applySectionClip();
    else clearSectionClip();
  };

  window.__section = {
    refresh: function () { if (sectionOn) applySectionClip(); }
  };
  ...
  var ui = {
    onNewQuestion: function (q, total, score) {
      ...
      el['feedback'].querySelector('.fb-mark').textContent = '';
    ...
    showFeedback: function (isC, std, correctStd) {
      ...
      f.classList.toggle('fb-bad', !isC);
      f.style.color = isC ? '#35d07f' : '#ff5b4d';
      f.querySelector('.fb-mark').textContent = isC ? '◎' : '×';
      var fl = el['flash'];
      fl.classList.toggle('bad', !isC);
  ...
  window.__dbg = { camera: camera, world: world };
