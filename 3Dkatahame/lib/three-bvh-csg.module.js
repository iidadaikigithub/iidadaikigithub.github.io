...
// relative tolerance factor 窶・multiplied by the max absolute coordinate
...
// Clips a line segment to the interior of a coplanar triangle using the Cyrus窶釘eck algorithm
...
function clipSegmentToTriangle( segment, tri, normal, target ) {
	...
	for ( let i = 0; i < 3; i ++ ) {
		...
		if ( Math.abs( denom ) < PARALLEL_EPSILON ) {
			...
			// segment parallel to edge 窶・entirely inside or outside this half-plane
...
function performSplitTriangleOperations(
	...
	for ( let i = 0, l = splitIds.length; i < l; i ++ ) {
		...
		if ( splitter.addConstraintEdge ) {
			...
			// edges are already in the common frame (brush A's local) 窶・no transform needed
